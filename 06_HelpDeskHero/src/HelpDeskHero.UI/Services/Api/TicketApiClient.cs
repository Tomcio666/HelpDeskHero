...
public sealed class TicketApiClient
{
    ...
    public Task<IReadOnlyList<TicketDto>?> GetDeletedAsync(CancellationToken ct = default) =>
        _http.GetFromJsonAsync<IReadOnlyList<TicketDto>>("api/tickets/deleted", ct);
    ...
    public async Task<HttpResponseMessage> RestoreAsync(int id, CancellationToken ct = default) =>
        await _http.PostAsync($"api/tickets/{id}/restore", null, ct);

    public string GetExportUrl(string? status, string? priority)
    {
        return
            $"api/tickets/export?status={Uri.EscapeDataString(status ?? string.Empty)}" +
            $"&priority={Uri.EscapeDataString(priority ?? string.Empty)}";
    }
}